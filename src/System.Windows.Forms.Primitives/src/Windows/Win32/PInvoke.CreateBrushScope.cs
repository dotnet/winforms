...
namespace Windows.Win32
{
    internal static partial class PInvoke
    {
...
#endif
        {
            ...
            /// <summary>
            ///  Creates a solid brush based on the <paramref name="color"/> using <see cref="PInvoke.CreateSolidBrush(uint)"/>.
            /// </summary>
            public CreateBrushScope(Color color)
            {
                HBRUSH = color.IsSystemColor
                    ? User32.GetSysColorBrush(color)
                    : CreateSolidBrush((uint)ColorTranslator.ToWin32(color));
                ValidateBrushHandle();
            }
