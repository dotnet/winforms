...
public class DataGridViewComboBoxCellTests : IDisposable
{
    ...
    private sealed class TestHuman : INotifyPropertyChanged
    {
        private TestKitten? _kitten;

        public TestKitten? Kitten
        {
            get => _kitten;
            set
            {
                if (!ReferenceEquals(_kitten, value))
                {
                    _kitten = value;
                    OnPropertyChanged();
    ...
    private sealed class TestKitten
    {
        public string? Name { get; set; }

        public string? Id { get; set; }

        public TestKitten Self => this;

        public string? DisplayName => Name;
    }

    private static DataGridView CreateGrid(
        BindingList<TestHuman> humans,
        BindingList<TestKitten> kittens,
        bool addDependentColumn = false)
    {
        ...
        {
            DataPropertyName = nameof(TestHuman.Kitten),
            DisplayMember = nameof(TestKitten.DisplayName),
            ValueMember = nameof(TestKitten.Self),
            ValueType = typeof(TestKitten),
            DataSource = kittens
        };

        grid.Columns.Add(comboColumn);

        if (addDependentColumn)
        {
            grid.Columns.Add(new DataGridViewTextBoxColumn
            {
                DataPropertyName = nameof(TestHuman.KittenId),
                Name = "KittenId"
            });
        }

        grid.DataSource = humans;

        return grid;
    }

    private static Form CreateHostedForm(Control control)
    {
        Form form = new();
    ...
    [WinFormsFact]
    public void DataGridViewComboBoxCell_EndEdit_DuplicateDisplayMember_SelectSecond_CommitsSecondObject()
    {
        BindingList<TestKitten> kittens = new(new[]
        {
            new TestKitten { Id = "1", Name = "Kitty1" },
            new TestKitten { Id = "2", Name = "Kitty1" },
            new TestKitten { Id = "3", Name = "Kitty1" }
        });

        BindingList<TestHuman> humans = new()
        {
            new TestHuman { Kitten = kittens[0] }
        };

        using DataGridView grid = CreateGrid(humans, kittens);
        using Form form = CreateHostedForm(grid);
        ...
        ComboBox editingControl = Assert.IsType<DataGridViewComboBoxEditingControl>(grid.EditingControl);
        editingControl.SelectedItem = kittens[1];

        grid.NotifyCurrentCellDirty(true);

        Assert.True(grid.EndEdit());

        Assert.Same(kittens[1], humans[0].Kitten);
        Assert.Equal("2", humans[0].KittenId);
    }

    [WinFormsFact]
    public void DataGridViewComboBoxCell_EndEdit_DuplicateDisplayMember_SelectThird_CommitsThirdObject()
    {
        BindingList<TestKitten> kittens = new(new[]
        {
            new TestKitten { Id = "1", Name = "Kitty1" },
        ...
        ComboBox editingControl = Assert.IsType<DataGridViewComboBoxEditingControl>(grid.EditingControl);
        editingControl.SelectedItem = kittens[2];

        grid.NotifyCurrentCellDirty(true);

        Assert.True(grid.EndEdit());

        Assert.Same(kittens[2], humans[0].Kitten);
        Assert.Equal("3", humans[0].KittenId);
    }

    [WinFormsFact]
    public void DataGridViewComboBoxCell_EndEdit_DuplicateDisplayMember_UpdatesDependentProperty()
    {
        BindingList<TestKitten> kittens = new(new[]
        {
            new TestKitten { Id = "1", Name = "Kitty1" },
        ...
        using DataGridView grid = CreateGrid(humans, kittens, addDependentColumn: true);
        using Form form = CreateHostedForm(grid);
        ...
        Assert.Equal("2", humans[0].KittenId);
        Assert.Equal("2", grid.Rows[0].Cells[1].Value);
    }
}
