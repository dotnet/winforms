    ...
    public class ComponentTray : ScrollableControl, IExtenderProvider, ISelectionUIHandler, IOleDragClient
    {
        ...
        private void OnContextMenu(int x, int y, bool useSelection)
        {
            if (!TabOrderActive)
            {
                ...
                if (mcs is not null)
                {
                    ...
                    if (useSelection && s is not null && !(1 == s.SelectionCount && s.PrimarySelection == mainDesigner.Component))
                    {
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing && controls is not null)
            {
                ...
                if (privateCommandSet is not null)
                {
        ...
        internal class TrayControl : Control
        {
            ...
            private void OnSetCursor()
            {
                ...
                if (prop is not null && ((bool)prop.GetValue(_component)) == true)
                {
                    Cursor.Current = Cursors.Default;
                    return;
                }
