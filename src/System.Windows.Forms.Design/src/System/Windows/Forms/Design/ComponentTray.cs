...
namespace System.Windows.Forms.Design
{
    ...
    public class ComponentTray : ScrollableControl, IExtenderProvider, ISelectionUIHandler, IOleDragClient
    {
        ...
        private MenuCommand _menucmdArrangeIcons = null;
        private MenuCommand _menucmdLineupIcons = null;
        private MenuCommand _menucmdLargeIcons = null;
        private bool _fResetAmbient = false;
        private bool _fSelectionChanged = false;
        private ComponentTrayGlyphManager glyphManager; //used to manage any glyphs added to the tray

        /// <summary>
        /// Creates a new component tray. The component tray will monitor component additions and removals and create appropriate UI objects in its space.
        /// </summary>
        ...
        public ComponentTray(IDesigner mainDesigner, IServiceProvider serviceProvider)
        {
            ...
            if (mcs != null)
            {
                Debug.Assert(_menucmdArrangeIcons == null, "Non-Null Menu Command for ArrangeIcons");
                Debug.Assert(_menucmdLineupIcons == null, "Non-Null Menu Command for LineupIcons");
                Debug.Assert(_menucmdLargeIcons == null, "Non-Null Menu Command for LargeIcons");
        ...
        /// <summary>
        /// Internally exposes a way for sited components to add glyphs to the component tray.
        /// </summary>
        ...
        private void DoAutoArrange(bool dirtyDesigner)
        {
            ...
            //This way, when OnLayout gets fired after this, we won't have to move every component again.  Note that sync'ing the selection will automatically select & scroll into view the right components
            ...
            try
            {
                ...
                foreach (Control ctl in controls)
                {
                    if (!ctl.Visible)
                        continue;
                    // If we're auto arranging, always move the control.  If not, move the control only if it was never given a position.
                    //his auto arranges it until the user messes with it, or until its position is saved into the resx. (if one control is no longer positioned, move all the other one as  we don't want them to go under one another)
                    if (autoArrange)
                    {
        ...
        /// If it returns true, then the component will get a glyph in the tray area.  If it returns false, then the component will not actually be added to the tray.
        /// The default implementation looks for DesignTimeVisibleAttribute.Yes on the component's class.
        /// </summary>
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing && controls != null)
            {
                ...
                if (mcs != null)
                {
                    Debug.Assert(_menucmdArrangeIcons != null, "Null Menu Command for ArrangeIcons");
                    Debug.Assert(_menucmdLineupIcons != null, "Null Menu Command for LineupIcons");
                    Debug.Assert(_menucmdLargeIcons != null, "Null Menu Command for LargeIcons");
        ...
        /// <summary>
        /// Similar to GetNextControl on Control, this method returns the next component in the tray, given a starting component.
        /// It will return null if the end (or beginning, if forward is false) of the list is encountered.
        /// </summary>
        ...
        /// <summary>
        /// Accessor method for the location extender property.  We offer this extender to all non-visual components.
        /// </summary>
        [Category("Layout")]
        [Localizable(false)]
        [Browsable(false)]
        ...
        /// <summary>
        /// Returns the traycontrol representing the IComponent.  If no traycontrol is found, this returns null.  This is used identify bounds for the DesignerAction UI.
        /// </summary>
        ...
        /// <summary>
        /// Called from CommandSet's OnMenuPaste method.  This will allow us to properly adjust the location of the components in the tray after we've incorreclty set them by deserializing the design time properties (and hence called SetValue(c, myBadLocation) on the location property).
        /// </summary>
        ...
        private class TraySelectionUIHandler : SelectionUIHandler
        {
            ...
            protected override IComponent GetComponent() => _tray;

            protected override Control GetControl() => _tray;

            protected override Control GetControl(IComponent component) => TrayControl.FromComponent(component);

            protected override Size GetCurrentSnapSize() => _snapSize;

            protected override object GetService(Type serviceType) => _tray.GetService(serviceType);

            protected override bool GetShouldSnapToGrid() => false;

            public override Rectangle GetUpdatedRect(Rectangle originalRect, Rectangle dragRect, bool updateSize) => dragRect;

            public override void SetCursor() => _tray.OnSetCursor();
        }
        ...
        /// <summary>
        /// This is called when we lose capture.  Here we get rid of any rubber band we were drawing.
        /// You should put any cleanup code in here.
        /// </summary>
        ...
        internal class NoResizeHandleGlyph : SelectionGlyphBase
        {
            ...
            internal NoResizeHandleGlyph(Rectangle controlBounds, SelectionRules selRules, bool primarySelection, Behavior.Behavior behavior) : base(behavior)
            {
        ...
        /// <summary>
        /// Sets the cursor. You may override this to set your own cursor.
        /// </summary>
        ...
        internal class TrayControl : Control
        {
            ...
            /// </summary>
            // CONSIDER: this method gets called three or four times per component, and is even reentrant (CreateGraphics can force handle creation, and OnCreateHandle calls this method).
            // There's probably a better way to do this, but since this doesn't seem to be on the critical path, I'm not going to lose sleep over it.
            ...
            {
                return new TrayControlAccessibleObject(this, _tray);
            }
            ...
            private void OnSetCursor()
            {
                ...
                {
                    // In case an unhandled exception was encountered, we don't want to leave the cursor with some strange shape
                    // Currently we have watson logs with FileNotFoundException only, so we are scoping the catch only to that type.
                    Cursor.Current = Cursors.Default;
