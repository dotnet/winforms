...
namespace System.Windows.Forms.Design
{
    ...
    internal static class DesignerUtils
    {
        ...
        private static HBRUSH s_grabHandleFillBrushPrimary =
            PInvoke.CreateSolidBrush((uint)ColorTranslator.ToWin32(SystemColors.Window));
        private static HBRUSH s_grabHandleFillBrush =
            PInvoke.CreateSolidBrush((uint)ColorTranslator.ToWin32(SystemColors.ControlText));
        private static HPEN s_grabHandlePenPrimary =
            PInvoke.CreatePen(PEN_STYLE.PS_SOLID, cWidth: 1, (uint)ColorTranslator.ToWin32(SystemColors.ControlText));
        private static HPEN s_grabHandlePen =
            PInvoke.CreatePen(PEN_STYLE.PS_SOLID, cWidth: 1, (uint)ColorTranslator.ToWin32(SystemColors.Window));
        ...
        public static void SyncBrushes()
        {
            ...
            s_grabHandleFillBrushPrimary = PInvoke.CreateSolidBrush((uint)ColorTranslator.ToWin32(SystemColors.Window));

            PInvoke.DeleteObject(s_grabHandleFillBrush);
            s_grabHandleFillBrush = PInvoke.CreateSolidBrush((uint)ColorTranslator.ToWin32(SystemColors.ControlText));

            PInvoke.DeleteObject(s_grabHandlePenPrimary);
            s_grabHandlePenPrimary = PInvoke.CreatePen(PEN_STYLE.PS_SOLID, cWidth: 1, (uint)ColorTranslator.ToWin32(SystemColors.ControlText));

            PInvoke.DeleteObject(s_grabHandlePen);
            s_grabHandlePen = PInvoke.CreatePen(PEN_STYLE.PS_SOLID, cWidth: 1, (uint)ColorTranslator.ToWin32(SystemColors.Window));
        }
