...
internal sealed partial class DropSourceBehavior : Behavior, IComparer
{
    ...
    internal DropSourceBehavior(List<Control> dragComponents, Control source, Point initialMouseLocation)
    {
        ...
        this._dragComponents = new DragComponent[dragComponents.Count];
    ...
    private Point AdjustToGrid(Point dragLoc)
    {
        ...
        Point controlLocation = new Point(dragLoc.X - _parentLocation.X, dragLoc.Y - _parentLocation.Y);
                ...
                try
                {
                    ...
                    if (performCopy)
                    {
                        ...
                        for (int j = 0; j < temp.Count; j++)
                        {
                            ...
                            _dragComponents[j].dragComponent = (Control)temp[j];
                        }
                    }
                    ...
                    for (int i = 0; i < _dragComponents.Length; i++)
                    {
                        ...
                        Point dropPoint = new Point(initialDropPoint.X + _dragComponents[i].positionOffset.X,
    ...
    internal void GiveFeedback(object sender, GiveFeedbackEventArgs e)
    {
        ...
        if ((mouseLoc != _lastFeedbackLocation) || (altKeyPressed && _dragAssistanceManager is not null))
        {
            ...
            Point newPosition = new Point(mouseLoc.X - _initialMouseLoc.X + _dragComponents[_primaryComponentIndex].originalControlLocation.X,
    ...
    private void InitiateDrag(Point initialMouseLocation, List<Control> dragComps)
    {
        ...
        for (int i = 0; i < _dragObjects.Length; i++)
        {
            ...
            _dragComponents[i].dragComponent = dragControl;
            ...
            DesignerUtils.GenerateSnapShot(dragControl, ref _dragComponents[i].dragImage, i == 0 ? 2 : 1, 1, backColor);
        ...
        for (int i = 0; i < _dragComponents.Length; i++)
        {
            if (primaryControl.Equals(_dragComponents[i].dragComponent))
            {
        ...
        using (Graphics g = Graphics.FromImage(_dragImage))
        {
            using (SolidBrush brush = new SolidBrush(primaryControl.BackColor))
            {
                for (int i = 0; i < _dragComponents.Length; i++)
                {
                    Rectangle controlRect = new Rectangle(_dragComponents[i].draggedLocation.X - _dragImageRect.X,
