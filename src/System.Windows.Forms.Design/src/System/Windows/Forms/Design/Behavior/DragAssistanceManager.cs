...
{
    /// <summary>
    /// The DragAssistanceManager, for lack of a better name, is responsible for integrating SnapLines into the DragBehavior.
    /// At the beginning of a DragBehavior this class is instantiated and at every mouse move this class is called and given the opportunity to adjust the position of the drag.
    /// The DragAssistanceManager needs to work as fast as possible - so not to interupt a drag operation.
    /// Because of this, this class has many global variables that are re-used, in hopes to limit the # of allocations per mouse move / drag operation.
    /// Also, for loops are used extensively (instead of foreach calls) to eliminate the creation of an enumerator.
    /// </summary>    
    ...
    internal sealed class DragAssistanceManager
    {
        ...
        // These are global lists of all the existing vertical and hoirizontal snaplines on the designer's surface excluding the targetControl.
        // All SnapLine coords in these lists have been properly adjusted for the AdornerWindow coords.
        ...
        // - we can: 1) remove anything > than snapDistance and 2) determine the smallest distance overall
        private int[] _verticalDistances;
        private int[] _horizontalDistances;

        // These are cleared and populated on every mouse move. These lists contain all the new vertical and horizontal lines we need to draw.
        ...
        // When we draw snap lines - we only draw lines from the targetControl to the control we're snapping to.  To do this, we'll keep a hashtable...
        ...
        /// Here it is assumed that all painting will be done to the AdornerWindow and that there are no target controsl to exclude from snapping.
        /// </summary>
        ...
        /// <summary>
        /// Internal constructor that takes the service provider, the list of dragCompoents, and a boolean indicating that we are resizing.
        /// </summary>
        ...
        /// <summary>
        /// Adjusts then adds each snap line the designer has to offer to either our global horizontal and vertical lists or our target lists.
        /// Note that we also keep track of our target snapline types - 'cause we can safely ignore all other types.  If valid target is false- then we don't yet know what we're snapping against - so we'll exclude the check below to skip unwanted snap line types.
        /// </summary>
        ...
        /// <summary>
        /// This internal method returns a snap line[] representing the last SnapLines that were rendered before this algorithm was stopped (usually by an OnMouseUp).
        ...
        internal Point OffsetToNearestSnapLocation(Control targetControl, Point directionOffset)
        {
            ...
            { // movement somewhere in the x dir first, build up our distance array
            ...
            { // movement somewhere in the y dir first, build up our distance array
        ...
        /// <summary>
        /// This function validates a Margin or Padding SnapLine.  A valid Margin SnapLine is one that will be drawn only if the target control being dragged somehow intersects (vertically or horizontally) the coords of the given snapLine.
        /// This is done so we don't start drawing margin lines when controls are large distances apart (too much mess);
        /// </summary>        
        [SuppressMessage("Microsoft.Performance", "CA1808:AvoidCallsThatBoxValueTypes")]
        ...
        private Point OnMouseMove(Rectangle dragBounds, bool offsetSnapLines, ref bool didSnap, bool shouldSnapHorizontally)
        {
            ...
            // First pass - build up a distance array of all same-type-alignment pts to the closest point on our targetControl.  Also, keep track of the smallest distance overall
