...
public sealed class ToolStripEditorManagerTests : IDisposable
{
    ...
    public void ToolStripEditorControl_OnWrappedEditorResize_DoesNotThrow()
    {
        ...
        MethodInfo? methodInfo = _toolStripEditorControl.GetType().GetMethod("OnWrappedEditorResize", BindingFlags.NonPublic | BindingFlags.Instance);

        Action act = () => methodInfo?.Invoke(_toolStripEditorControl, [_toolStripEditorControl, eventArgs]);
        act.Should().NotThrow();
    }
}
