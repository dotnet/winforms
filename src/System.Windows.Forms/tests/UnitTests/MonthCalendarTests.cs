...
namespace System.Windows.Forms.Tests
{
    public class MonthCalendarTests
    {
        ...
        public void MonthCalendar_Ctor_Default()
        {
            ...
            Assert.False(calendar.AllowDrop);
            Assert.Equal(AnchorStyles.Top | AnchorStyles.Left, calendar.Anchor);
            Assert.Empty(calendar.AnnuallyBoldedDates);
            Assert.NotSame(calendar.AnnuallyBoldedDates, calendar.AnnuallyBoldedDates);
            Assert.False(calendar.AutoSize);
            Assert.Equal(SystemColors.Window, calendar.BackColor);
            Assert.Null(calendar.BackgroundImage);
            Assert.Equal(ImageLayout.Tile, calendar.BackgroundImageLayout);
            Assert.Null(calendar.BindingContext);
            Assert.Empty(calendar.BoldedDates);
            Assert.NotSame(calendar.BoldedDates, calendar.BoldedDates);
            Assert.True(calendar.Bottom > 0);
            Assert.Equal(0, calendar.Bounds.X);
            Assert.Equal(0, calendar.Bounds.Y);
            Assert.True(calendar.Bounds.Width > 0);
            Assert.True(calendar.Bounds.Height > 0);
            Assert.Equal(new Size(1, 1), calendar.CalendarDimensions);
            Assert.False(calendar.CanEnableIme);
            Assert.True(calendar.CanRaiseEvents);
            Assert.True(calendar.CausesValidation);
            Assert.True(calendar.ClientSize.Width > 0);
            Assert.True(calendar.ClientSize.Height > 0);
            Assert.Equal(0, calendar.ClientRectangle.X);
            Assert.Equal(0, calendar.ClientRectangle.Y);
            Assert.True(calendar.ClientRectangle.Width > 0);
            Assert.True(calendar.ClientRectangle.Height > 0);
            Assert.Null(calendar.Container);
            Assert.Null(calendar.ContextMenu);
            Assert.Null(calendar.ContextMenuStrip);
            Assert.Empty(calendar.Controls);
            Assert.Same(calendar.Controls, calendar.Controls);
            Assert.False(calendar.Created);
            Assert.Same(Cursors.Default, calendar.Cursor);
            Assert.Same(Cursors.Default, calendar.DefaultCursor);
            Assert.Equal(ImeMode.Disable, calendar.DefaultImeMode);
            Assert.Equal(new Padding(9), calendar.DefaultMargin);
            Assert.Equal(Size.Empty, calendar.DefaultMaximumSize);
            Assert.Equal(Size.Empty, calendar.DefaultMinimumSize);
            Assert.Equal(Padding.Empty, calendar.DefaultPadding);
            Assert.True(calendar.DefaultSize.Width > 0);
            Assert.True(calendar.DefaultSize.Height > 0);
            Assert.False(calendar.DesignMode);
            Assert.Equal(0, calendar.DisplayRectangle.X);
            Assert.Equal(0, calendar.DisplayRectangle.Y);
            Assert.True(calendar.DisplayRectangle.Width > 0);
            Assert.True(calendar.DisplayRectangle.Height > 0);
            Assert.Equal(DockStyle.None, calendar.Dock);
            Assert.False(calendar.DoubleBuffered);
            Assert.True(calendar.Enabled);
            Assert.NotNull(calendar.Events);
            Assert.Same(calendar.Events, calendar.Events);
            Assert.Equal(Day.Default, calendar.FirstDayOfWeek);
            Assert.Equal(Control.DefaultFont, calendar.Font);
            Assert.Equal(calendar.Font.Height, calendar.FontHeight);
            Assert.Equal(SystemColors.WindowText, calendar.ForeColor);
            Assert.False(calendar.HasChildren);
            Assert.True(calendar.Height > 0);
            Assert.Equal(ImeMode.Disable, calendar.ImeMode);
            Assert.Equal(ImeMode.Disable, calendar.ImeModeBase);
            Assert.Equal(0, calendar.Left);
            Assert.Equal(Point.Empty, calendar.Location);
            Assert.Equal(new Padding(9), calendar.Margin);
            Assert.Equal(new DateTime(9998, 12, 31), calendar.MaxDate);
            Assert.Equal(7, calendar.MaxSelectionCount);
            Assert.Equal(new DateTime(1753, 1, 1), calendar.MinDate);
            Assert.Empty(calendar.MonthlyBoldedDates);
            Assert.NotSame(calendar.MonthlyBoldedDates, calendar.MonthlyBoldedDates);
            Assert.Equal(Padding.Empty, calendar.Padding);
            Assert.Null(calendar.Parent);
            Assert.Equal("Microsoft\u00AE .NET", calendar.ProductName);
            Assert.False(calendar.RecreatingHandle);
            Assert.Null(calendar.Region);
            Assert.False(calendar.ResizeRedraw);
            Assert.True(calendar.Right > 0);
            Assert.Equal(RightToLeft.No, calendar.RightToLeft);
            Assert.False(calendar.RightToLeftLayout);
            Assert.Equal(0, calendar.ScrollChange);
            ...
            Assert.NotSame(calendar.SelectionRange, calendar.SelectionRange);
            ...
            Assert.Equal(new Size(176, 153), calendar.SingleMonthSize);
            Assert.Null(calendar.Site);
            Assert.True(calendar.Size.Width > 0);
            Assert.True(calendar.Size.Height > 0);
            Assert.Equal(0, calendar.TabIndex);
            Assert.True(calendar.TabStop);
            Assert.Empty(calendar.Text);
            Assert.Equal(SystemColors.ActiveCaption, calendar.TitleBackColor);
            Assert.Equal(SystemColors.ActiveCaptionText, calendar.TitleForeColor);
            Assert.Equal(DateTime.Now.Date, calendar.TodayDate);
            Assert.False(calendar.TodayDateSet);
            Assert.Equal(0, calendar.Top);
            Assert.Equal(SystemColors.GrayText, calendar.TrailingForeColor);
            Assert.True(calendar.Visible);
            Assert.True(calendar.Width > 0);
