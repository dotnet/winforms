...
namespace System.Windows.Forms.Tests
{
    ...
    public class ListBoxTests : IClassFixture<ThreadExceptionFixture>
    {
        ...
        public void ListBox_ColumnWidth_GetItemRect_ReturnsExpected()
        {
            ...
            RECT rc = default;
            Assert.Equal(1, SendMessageW(control, (WM)LB.GETITEMRECT, 0, ref rc));
        ...
        public void ListBox_HorizontalExtent_GetHorizontalExtent_Success(bool multiColumn, bool horizontalScrollBar, int expected)
        {
            ...
            Assert.NotEqual(IntPtr.Zero, control.Handle);
            Assert.Equal(0, control.HorizontalExtent);
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETHORIZONTALEXTENT));

            control.HorizontalExtent = 10;
            Assert.Equal(expected, SendMessageW(control, (WM)LB.GETHORIZONTALEXTENT));
        }
        ...
        public void ListBox_ItemHeight_Set_GetItemHeight_ReturnsExpected(DrawMode drawMode, bool expected)
        {
            ...
            Assert.NotEqual(IntPtr.Zero, control.Handle);
            Assert.Equal(expected, SendMessageW(control, (WM)LB.GETITEMHEIGHT) == 25);
        }
        ...
        public void ListBox_SelectedIndex_GetCurSelOne_Success()
        {
            ...
            Assert.NotEqual(IntPtr.Zero, control.Handle);
            control.SelectedIndex = 1;
            Assert.Equal(1, SendMessageW(control, (WM)LB.GETCURSEL));

            // Select first.
            control.SelectedIndex = 0;
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETCURSEL));
        ...
        public void ListBox_SelectedIndex_GetCurSelMultiple_Success(SelectionMode selectionMode)
        {
            ...
            Assert.Equal(1, SendMessageW(control, (WM)LB.GETSELITEMS, buffer.Length, ref buffer[0]));
            Assert.Equal(new int[] { 1, 0, 0, 0, 0 }, buffer.ToArray());

            // Select first.
            control.SelectedIndex = 0;
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETCURSEL));
            Assert.Equal(2, SendMessageW(control, (WM)LB.GETSELITEMS, buffer.Length, ref buffer[0]));
            Assert.Equal(new int[] { 0, 1, 0, 0, 0 }, buffer.ToArray());

            // Clear selection.
            control.SelectedIndex = -1;
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETCURSEL));
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETSELITEMS, buffer.Length, ref buffer[0]));
            Assert.Equal(new int[] { 0, 1, 0, 0, 0 }, buffer.ToArray());
        }
        ...
        public void ListBox_SelectedItems_GetDirtyCustom_ReturnsExpected()
        {
            ...
            void Dirty()
            {
                ...
                SendMessageW(control, WM.REFLECT | WM.COMMAND, PARAM.FromLowHigh(0, (int)LBN.SELCHANGE));
            }
        }
        ...
        public void ListBox_TopIndex_GetTopIndex_ReturnsExpected()
        {
            ...
            Assert.NotEqual(IntPtr.Zero, control.Handle);
            control.TopIndex = 1;
            Assert.Equal(0, SendMessageW(control, (WM)LB.GETTOPINDEX));
        }
