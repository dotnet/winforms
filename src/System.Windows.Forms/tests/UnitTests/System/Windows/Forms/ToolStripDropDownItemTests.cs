...
public class ToolStripItemDropDownTests
{
    [WinFormsFact]
    public void ToolStripDropDownItem_Ctor_Default()
    {
        using SubToolStripDropDownItem item = new();
    ...
    {
        using SubToolStripDropDownItem item = new(text, image, onClick);
        Assert.NotNull(item.AccessibilityObject);
        Assert.Null(item.AccessibleDefaultActionDescription);
        Assert.Null(item.AccessibleDescription);
        Assert.Null(item.AccessibleName);
    ...
    public void ToolStripDropDownItem_Ctor_String_Image_EventHandler_InvokeClick_CallsOnClick()
    {
        ...
        using SubToolStripDropDownItem item = new("text", null, onClick);
        item.PerformClick();
    ...
    {
        using SubToolStripDropDownItem item = new(text, image, onClick, name);
        Assert.NotNull(item.AccessibilityObject);
        Assert.Null(item.AccessibleDefaultActionDescription);
        Assert.Null(item.AccessibleDescription);
        Assert.Null(item.AccessibleName);
    ...
    public void ToolStripDropDownItem_Ctor_String_Image_EventHandler_String_InvokeClick_CallsOnClick()
    {
        int callCount = 0;
        EventHandler onClick = (sender, e) => callCount++;
        using SubToolStripDropDownItem item = new("text", null, onClick, "name");
        item.PerformClick();
        Assert.Equal(1, callCount);
    }
    ...
    {
        using SubToolStripDropDownItem item = new(text, image, dropDownItems);
    ...
    {
        using ToolStripDropDown result = new();
        using CustomCreateDefaultDropDownToolStripDropDownItem item = new()
        {
