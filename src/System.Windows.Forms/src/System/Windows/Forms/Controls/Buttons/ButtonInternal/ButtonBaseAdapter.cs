...
internal abstract partial class ButtonBaseAdapter
{
    ...
    {
        if (Control.BackColor != Application.SystemColors.Control && SystemInformation.HighContrast)
        {
            if (raised)
            {
    ...
    {
        bool stockColor = colors.ButtonFace.ToKnownColor() == Application.SystemColors.Control.ToKnownColor();
        ...
        using CreatePenScope penTopLeft = new(
            ...
            : stockColor ? Application.SystemColors.ControlLightLight : colors.Highlight);
        ...
        using CreatePenScope penBottomRight = new(
            disabledHighContrast
                ? colors.WindowDisabled
                : stockColor ? Application.SystemColors.ControlDarkDark : colors.ButtonShadowDark);
        ...
        using CreatePenScope insetPen = new(
            stockColor
                ? SystemInformation.HighContrast ? Application.SystemColors.ControlLight : Application.SystemColors.Control
        ...
        using CreatePenScope bottomRightInsetPen = new(
            disabledHighContrast
            ? colors.WindowDisabled
            : stockColor ? Application.SystemColors.ControlDark : colors.ButtonShadow);
    ...
    private static void Draw3DBorderNormal(IDeviceContext deviceContext, ref Rectangle bounds, ColorData colors)
    {
        ...
        // Bottom + right inset
        using CreatePenScope insetPen = new(
            colors.ButtonFace.ToKnownColor() == Application.SystemColors.Control.ToKnownColor()
