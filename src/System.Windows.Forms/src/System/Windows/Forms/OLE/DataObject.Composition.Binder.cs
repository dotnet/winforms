...
/// <summary>
///  A type resolver for use in the <see cref="Composition.NativeToWinFormsAdapter"/> when processing binary formatted stream
///  contained in our <see cref="DataObject"/> class using the typed consumption side APIs, such as
///  <see cref="Composition.TryGetData{T}(out T)"/>. This class recognizes primitive types, exchange types from
...
///  </para>
/// </remarks>
internal sealed class TypeBinder : SerializationBinder, ITypeResolver
{
    ...
    ///  <see cref="Composition.GetData(string)"/> API family, that returns an <see cref="object"/>. <see langword="false"/>
    ///  if the user had requested a specific type by calling <see cref="Composition.TryGetData{T}(out T)"/> API family.
    /// </param>
    public TypeBinder(Type type, Func<TypeName, Type>? resolver, bool legacyMode)
    {
    ...
    public override Type? BindToType(string assemblyName, string typeName)
    {
        ...
        if (_legacyMode)
        {
            return CoreAppContextSwitches.ClipboardDragDropEnableUnsafeBinaryFormatterSerialization
                ? null
