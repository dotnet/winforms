...
public interface IDataObject
{
    ...
    ///  <paramref name="resolver"/> is implemented by the user and should return the allowed type or
...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    ...
    {
        data = default;
        if (!DataObject.ValidateGetDataArguments<T>(format, resolver))
        {
            return false;
        }
    ...
    /// <remarks>
    ///  <para>
    ///    This overload will not get types that require <see cref="BinaryFormatter"/>.
    ///  </para>
    /// </remarks>
    bool TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
        string format,
