        ...
        private class DropDownHolder : Form, IMouseHookClient
        {
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                else if (m.Msg == WindowMessages.WM_DPICHANGED)
                {
                    ...
                    var oldDpi = this._deviceDpi;
                    this._deviceDpi = (int)UnsafeNativeMethods.GetDpiForWindow(new HandleRef(this, HandleInternal));
                    if (oldDpi != _deviceDpi)
                    {
                        RescaleConstantsForDpi(oldDpi, this._deviceDpi);
                        this.PerformLayout();
                    }
