...
namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    ...
    /// </summary>
    internal unsafe abstract class BaseCAMarshaler<T> where T : unmanaged
    {
        private static readonly TraceSwitch s_caMarshalSwitch = new ("CAMarshal", "BaseCAMarshaler: Debug CA* struct marshaling");

        private void* _caArrayAddress;
        private readonly uint _count;
        private object[] _itemArray;

        protected BaseCAMarshaler(in Ole32.CA caStruct)
        {
            _count = caStruct.cElems;
            _caArrayAddress = caStruct.pElems;
            Debug.WriteLineIf(s_caMarshalSwitch.TraceVerbose, $"Marshaling {_count} items of type {ItemType.Name}");
        }
        ...
        public virtual object[] Items
        {
            get
            {
                ...
                {
                    Debug.WriteLineIf(s_caMarshalSwitch.TraceVerbose, $"Marshaled: {_itemArray.Length} items, array type={_itemArray.GetType().Name}");
                }
#endif
                return _itemArray;
            }
        ...
        /// <summary>
        ///  Override this member to convert the array item into the desired type.
        /// </summary>
        protected abstract object GetItem(T item);
        ...
        private object[] GetItems()
        {
            ...
            var nativeItems = new ReadOnlySpan<T>(_caArrayAddress, (int)_count);
            for (int i = 0; i < nativeItems.Length; i++)
            {
                try
                {
                    object curItem = GetItem(nativeItems[i]);
                    if (curItem is not null && ItemType.IsInstanceOfType(curItem))
                    {
                        items[i] = curItem;
                    }

                    Debug.WriteLineIf(s_caMarshalSwitch.TraceVerbose, $"Marshaled {ItemType.Name} item, value={curItem?.ToString() ?? "(null)"}");
                }
                catch (Exception ex)
                {
