...
internal sealed unsafe class TreeViewLabelEditUiaTextProvider : UiaTextProvider
{
    /// <summary>
    ///  Since the label edit inside the TreeView is always single-line, for optimization
    ...
    public TreeViewLabelEditUiaTextProvider(TreeView owner, TreeViewLabelEditNativeWindow childEdit, AccessibleObject childEditAccessibilityObject)
    {
        _owningTreeView = owner.OrThrowIfNull();
    ...
    public override LOGFONTW Logfont => LOGFONTW.FromFont(_owningTreeView.Font);
    ...
    public override void GetVisibleRangePoints(out int visibleStart, out int visibleEnd)
    {
        ...
        if (IsDegenerate(_owningTreeView.ClientRectangle))
        {
            return;
        }
