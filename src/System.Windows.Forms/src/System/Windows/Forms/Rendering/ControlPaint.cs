...
public static unsafe partial class ControlPaint
{
    ...
    public static Color ContrastControlDark
        => SystemInformation.HighContrast ? Application.ApplicationColors.WindowFrame : Application.ApplicationColors.ControlDark;
    ...
    private static void DrawBorderComplex(Graphics graphics, Rectangle bounds, Color color, ButtonBorderStyle style)
    {
        ...
        if (style == ButtonBorderStyle.Inset)
        {
            ...
            // Bottom + right inset
            if (color.ToKnownColor() == Application.ApplicationColors.Control.ToKnownColor())
            {
        ...
        else
        {
            ...
            bool stockColor = color.ToKnownColor() == Application.ApplicationColors.Control.ToKnownColor();
            HLSColor hls = new(color);

            // Top + left
            using var lightPen = (stockColor ? Application.ApplicationColors.ControlLightLight : hls.Lighter(1.0f)).GetCachedPenScope();
            ...
            // Bottom + right
            using var darkPen = (stockColor ? Application.ApplicationColors.ControlDarkDark : hls.Darker(1.0f)).GetCachedPenScope();
            ...
            using var topLeftPen = (!stockColor
                ...
                : SystemInformation.HighContrast
                    ...
                    : Application.ApplicationColors.Control).GetCachedPenScope();
            ...
            // Bottom + right inset
            using var bottomRightPen = (stockColor ? Application.ApplicationColors.ControlDark : hls.Darker(0.5f)).GetCachedPenScope();
    ...
    private static void DrawFlatCheckBox(Graphics graphics, Rectangle rectangle, ButtonState state)
    {
        ...
        Color foreground = ((state & ButtonState.Inactive) == ButtonState.Inactive)
            ? (SystemInformation.HighContrast ? Application.ApplicationColors.GrayText : Application.ApplicationColors.ControlDark)
    ...
    public static void DrawFocusRectangle(Graphics graphics, Rectangle rectangle)
        => DrawFocusRectangle(graphics, rectangle, Application.ApplicationColors.ControlText, Application.ApplicationColors.Control);
        ...
        FONT_QUALITY quality = FONT_QUALITY.DEFAULT_QUALITY)
    {
        if (SystemInformation.HighContrast)
        {
            TextRenderer.DrawTextInternal(dc, s, font, layoutRectangle, Application.ApplicationColors.GrayText, quality, format);
        }
        else
        {
            layoutRectangle.Offset(1, 1);
    ...
    {
        Color brushColor = backColor.GetBrightness() <= .5 ? Application.ApplicationColors.ControlLight : Application.ApplicationColors.ControlDark;
