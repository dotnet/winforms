        ...
        public class ListViewSubItemCollection : IList
        {
            ...
            public ListViewSubItem this[int index]
            {
                get
                {
                    ...
                    return _owner._listViewSubItems[index];
                }
                set
                {
                    if (index < 0 || index >= Count)
                    ...
                    _owner._listViewSubItems[index] = value ?? throw new ArgumentNullException(nameof(value));
            ...
            private void EnsureSubItemSpace(int size, int index)
            {
                ...
                else
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        {
                            _owner._listViewSubItems[i + size] = _owner._listViewSubItems[i];
                        }
                    }
                }
            }
            ...
            public int IndexOf(ListViewSubItem subItem)
            {
                ...
                {
                    if (_owner._listViewSubItems[index] == subItem)
                    {
                        return index;
                    }
                }
            ...
            public void Insert(int index, ListViewSubItem item)
            {
                ...
                // Insert new item
                _owner._listViewSubItems[index] = item;
                _owner.SubItemCount++;
                _owner.UpdateSubItems(-1);
            }
            ...
            public void RemoveAt(int index)
            {
                ...
                _owner._listViewSubItems[index].owner = null;

                // Collapse the items
                for (int i = index + 1; i < _owner.SubItemCount; i++)
                {
                    _owner._listViewSubItems[i - 1] = _owner._listViewSubItems[i];
                }

                int oldCount = _owner.SubItemCount;
                _owner.SubItemCount--;
                _owner._listViewSubItems[_owner.SubItemCount] = null;
                _owner.UpdateSubItems(-1, oldCount);
            }
            ...
            void ICollection.CopyTo(Array dest, int index)
            {
                if (Count > 0)
                {
                    Array.Copy(_owner._listViewSubItems, 0, dest, index, Count);
                }
            }
