...
#Disable Warning CA1838 ' Avoid 'StringBuilder' parameters for P/Invokes
        ...
        <DllImport("kernel32", CharSet:=CharSet.Auto, PreserveSig:=True, BestFitMapping:=False, ThrowOnUnmappableChar:=True)>
        ...
        <DllImport("kernel32", CharSet:=CharSet.Auto, PreserveSig:=True, BestFitMapping:=False, ThrowOnUnmappableChar:=True)>
        Friend Sub GetStartupInfo(
            <[In](),
            Out()> lpStartupInfo As NativeTypes.STARTUPINFO)
        End Sub
