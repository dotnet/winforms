...
public class BlackSaturationCurveEffect : ColorCurveEffect
{
    ...
    ///  A value of t specifies that the interval [t, 255] is mapped linearly to the interval [0, 255].
    ///  For example, if <paramref name="blackSaturation"/> is equal to 15,
    ///  then color channel values in the interval [15, 255] are adjusted
    ///  so that they spread out over the interval [0, 255]. Color channel values less than 15 are set to 0.
    /// </param>
    /// <exception cref="ArgumentException"><paramref name="blackSaturation"/>
