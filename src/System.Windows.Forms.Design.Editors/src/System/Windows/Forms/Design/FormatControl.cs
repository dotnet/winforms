...
namespace System.Windows.Forms.Design
{
    internal partial class FormatControl : UserControl
    {
        ...
        public static string FormatTypeStringFromFormatString(string formatString)
        {
            ...
            else if (FormatControl.CurrencyFormatType.ParseStatic(formatString))
            {
                return SR.BindingFormattingDialogFormatTypeCurrency;
            }
            else if (FormatControl.DateTimeFormatType.ParseStatic(formatString))
            {
                return SR.BindingFormattingDialogFormatTypeDateTime;
            }
            else if (FormatControl.ScientificFormatType.ParseStatic(formatString))
            {
        ...
        {
            if (Control.IsMnemonic(charCode, formatTypeLabel.Text))
            {
                formatTypeListBox.Focus();
                return true;
            }

            if (Control.IsMnemonic(charCode, nullValueLabel.Text))
            {
            ...
            switch (selIndex)
            {
                ...
                // FALL THRU
                case NumericIndex:
                    Diagnostics.Debug.Assert(decimalPlacesUpDown.Visible);
                    if (Control.IsMnemonic(charCode, secondRowLabel.Text))
                    {
                ...
                case DateTimeIndex:
                    Diagnostics.Debug.Assert(dateTimeFormatsListBox.Visible);
                    if (Control.IsMnemonic(charCode, secondRowLabel.Text))
                    {
                ...
                case CustomIndex:
                    Diagnostics.Debug.Assert(customStringTextBox.Visible);
                    if (Control.IsMnemonic(charCode, secondRowLabel.Text))
                    {
        ...
        private void UpdateFormatTypeListBoxItems()
        {
            ...
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "d"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "D"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "f"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "F"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "g"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "G"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "t"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "T"));
            dateTimeFormatsListBox.Items.Add(new DateTimeFormatsListBoxItem(dateTimeFormatValue, "M"));
        ...
        // This method tells the BindingFormattingDialog and the FormatStringDialog that the FormatControl is loaded and resized.
        private void FormatControlFinishedLoading()
        {
            //BindingFormattingDialog bfd = null;
            FormatStringDialog fsd = null;

            //UNDONE: Is this the same as Control.TopLevelControl?
            Control ctl = Parent;
            while (ctl != null)
            {
        ...
        private class NumericFormatType : FormatTypeClass
        {
            ...
            public override string FormatString
            {
                get
                {
                    switch ((int)owner.decimalPlacesUpDown.Value)
                    {
                        case 0:
                            return "N0";
                        case 1:
            ...
            {
#if DEBUG
                Diagnostics.Debug.Assert(Parse(formatString), "we only push valid strings");
#endif // DEBUG
                if (formatString.Equals("N0"))
                {
                    owner.decimalPlacesUpDown.Value = 0;
                }
                else if (formatString.Equals("N1"))
                {
                    owner.decimalPlacesUpDown.Value = 1;
                }
                else if (formatString.Equals("N2"))
                {
                    owner.decimalPlacesUpDown.Value = 2;
                }
                else if (formatString.Equals("N3"))
                {
                    owner.decimalPlacesUpDown.Value = 3;
                }
                else if (formatString.Equals("N4"))
                {
                    owner.decimalPlacesUpDown.Value = 4;
                }
                else if (formatString.Equals("N5"))
                {
                    owner.decimalPlacesUpDown.Value = 5;
                }
                else if (formatString.Equals("N6"))
                {
                    owner.decimalPlacesUpDown.Value = 6;
                }
            }
                ...
                get
                {
                    switch ((int)owner.decimalPlacesUpDown.Value)
                    {
                        ...
                        case 6:
                            return "C6";
                        default:
                            Diagnostics.Debug.Fail("decimalPlacesUpDown should allow only up to 6 digits");
                            return "";
                    }
                }
            }
        ...
        private class DateTimeFormatType : FormatTypeClass
        {
            ...
            public override string SampleString
            {
                get
                {
                    if (owner.dateTimeFormatsListBox.SelectedItem == null)
                    {
                        return "";
                    }

                    return FormatControl.dateTimeFormatValue.ToString(FormatString, CultureInfo.CurrentCulture);
                }
            }
            public override bool DropDownVisible
            {
            ...
            public override string FormatString
            {
                get
                {
                    DateTimeFormatsListBoxItem item = owner.dateTimeFormatsListBox.SelectedItem as DateTimeFormatsListBoxItem;
            ...
            public override void PushFormatStringIntoFormatType(string formatString)
            {
                ...
                owner.dateTimeFormatsListBox.SelectedIndex = selectedIndex;
            }
            public override string ToString()
            {
                return SR.BindingFormattingDialogFormatTypeDateTime;
        ...
        private class CustomFormatType : FormatTypeClass
        {
            ...
            public override string SampleString
            {
                get
                {
                    ...
                    {
                        label = FormatControl.dateTimeFormatValue.ToString(formatString, CultureInfo.CurrentCulture);
                    }
            ...
            public override string FormatString
            {
                get
                {
                    return owner.customStringTextBox.Text;
                }
            }
            ...
            public override void PushFormatStringIntoFormatType(string formatString)
            {
                owner.customStringTextBox.Text = formatString;
            }
