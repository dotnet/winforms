...
- Code should be succinct (and as such we lean into new C# language features)
...
Windows Forms dates back to the mid 1990s (originally Windows Foundation Classes). It has seen over 25 years of language and coding evolution. We believe that updating the code to modern style makes the code easier to maintain and update in the longer term. While we do not update everything as the style guidelines evolve, we do make continual investments in cleaning the codebase as an engineering best practice.
...
1. Methods and properties should have XML comments. `public` API comments should align with the documentation on https://learn.microsoft.com.
...
1. Use PascalCasing to name all types. The only exception is for interop types, which should match the native casing.
...
1. Use `_camelCase` for internal and private fields and use `readonly` where possible. Prefix instance fields with `_`, static fields with `s_` and thread static fields with `t_`.
1. Use PascalCasing to name all constants. The only exception is for interop code where the constant value should match the naming in the native code.
1. `public` or `internal` fields should not be used (use properties).

### Properties

1. Use PascalCasing to name all properties.
...
1. Use PascalCasing to name all methods.
...
1. We never use `var` for intrinsic or base types, e.g. `var count = 2`. `var` should only be used when the type name inhibits code readability.
